...
public class ennemieComportement_rapide : MonoBehaviour
{
...
public static ennemieComportement_rapide instance; // Instance statique de la classe pour l'accès global
    private void Awake()
    {
        if (instance != null)
        {
            Debug.LogWarning("Il y a plus d'une instance de ennemieComportement_rapide dans la scène");
            return;
        }
  ...
  if (rb.bodyType == RigidbodyType2D.Dynamic)
{
    ...
    // Toujours l'animation "right"
    animators.SetBool("right", true);

    // Flip selon la direction horizontale
    if (dir.x >= 0)
    {
        spriteRenderer.flipX = false; // Droite
    }
    else
    {
        spriteRenderer.flipX = true; // Gauche
    }
}
    }
...
private void OnTriggerEnter2D(Collider2D collision)
{
    ...
    private IEnumerator death()
    {
       ...
       audioSource.Stop();
       yield return new WaitForSeconds(0.1f); // Attendre la durée de l'animation de mort avant de jouer le son
